...
const app = {
    ...
    async handleLogin() {
        ...
        try {
            ...
            if (navbarUser) {
                navbarUser.textContent = data.user.name || data.user.username;
            }
    ...
    logout() {
        ...
        if (navbarUsername) {
            navbarUsername.textContent = 'Admin';
        }
    ...
    async loadInitialData() {
        try {
            const [colabRes, clientesRes, proyRes, tiposRes] = await Promise.all([
                fetch('/api/config/colaboradores'),
                fetch('/api/config/clientes'),
                fetch('/api/config/proyectos'),
    ...
    renderViewerCells(allocations) {
        ...
        for (let i = 0; i < 5; i++) {
            ...
            html += `
                <div class="day-cell" style="flex: 1; min-width: 100px; border: 1px solid #eee; padding: 4px; border-radius: 4px; background: #fff;">
                    <div style="font-size: 10px; color: #999; text-align: center; margin-bottom: 4px; font-weight: bold;">${dayName} ${dayMonth}</div>
                    ${dayAllocations.map(alloc => `
                        <div style="margin-bottom: 2px; padding: 4px; background: #e3f2fd; border-radius: 3px; font-size: 11px; border-left: 3px solid #2196f3;">
                            <strong>${alloc.cliente_name}</strong>: ${alloc.hours}h
                        </div>
                    `).join('')}
                    ${roundedDayTotal > 0 ? `<div style="text-align: center; font-size: 10px; color: #666; margin-top: 4px; font-weight: bold;">Total: ${roundedDayTotal}h</div>` : ''}
                </div>
            `;
        }
    ...
    renderPlanningGrid(allocations) {
        ...
        Object.keys(byColaborador).forEach(colabId => {
            ...
            tdName.innerHTML = `
                ...
                <button onclick="app.deleteCollaboratorPlanning(${colabId}, '${colab.name.replace(/'/g, "\\'")}')" 
                        class="btn-icon text-red" title="Eliminar planificación de ${colab.name}">
        ...
        for (let i = 0; i < 5; i++) {
            ...
            html += `
                <div class="day-cell" style="flex: 1; min-width: 100px; border: 1px solid #eee; padding: 4px; border-radius: 4px; background: #fff;">
                    <div style="font-size: 10px; color: #999; text-align: center; margin-bottom: 4px; font-weight: bold;">${dayName} ${dayMonth}</div>
                    ${dayAllocations.map(alloc => `
                        <div onclick="app.openAllocationModal(${alloc.id}, null, null)" 
                             style="cursor: pointer; margin-bottom: 2px; padding: 4px; background: #e3f2fd; border-radius: 3px; font-size: 11px; border-left: 3px solid #2196f3;">
                            <strong>${alloc.cliente_name}</strong>: ${alloc.hours}h
                        </div>
                    `).join('')}
                    <div onclick="app.openAllocationModal(null, ${colaboradorId}, '${dateStr}')" 
                         style="cursor: pointer; text-align: center; color: #ccc; font-size: 10px; padding: 2px; margin-top: 4px; border: 1px dashed #eee; border-radius: 3px;">
                        +
                    </div>
                    ${roundedDayTotal > 0 ? `<div style="text-align: center; font-size: 10px; color: #666; margin-top: 4px; font-weight: bold;">Total: ${roundedDayTotal}h</div>` : ''}
                </div>
    ...
    renderConfigTable(table) {
            ...
            data.forEach(item => {
                ...
                tr.innerHTML = `
                    ...
                    <td style="text-align: right;">
                        <button class="btn-icon" onclick="app.editUser(${item.id})"><i class="fas fa-edit"></i></button>
                        <button class="btn-icon" onclick="app.deleteRecord('usuarios', ${item.id})"><i class="fas fa-trash"></i></button>
                    </td>
                `;
                tbody.appendChild(tr);
            });
        ...
        data.forEach(item => {
            ...
            tr.innerHTML = `
                ...
                <td>${item.id}</td>
                <td>${item.name}</td>
                ${table === 'clientes' ? `<td><span class="text-red">${item.proyecto_name || '-'}</span></td><td><span class="text-red">${item.tipo_name || '-'}</span></td>` : ''}
                <td style="text-align: right;">
                    <button class="btn-icon" onclick="app.editRecord('${table}', ${item.id})"><i class="fas fa-edit"></i></button>
                    <button class="btn-icon" onclick="app.deleteRecord('${table}', ${item.id})"><i class="fas fa-trash"></i></button>
                </td>
            `;
            tbody.appendChild(tr);
        });
    ...
    async deleteRecord(table, id) {
        ...
        try {
            await fetch(`/api/config/${table}/${id}`, { method: 'DELETE' });
            await this.loadInitialData();
            this.renderConfigTable(table);
        } catch (error) {
    ...
    async deleteSelected(table) {
        ...
        try {
            await Promise.all(Array.from(checkboxes).map(cb =>
                fetch(`/api/config/${table}/${cb.dataset.id}`, { method: 'DELETE' })
            ));
            await this.loadInitialData();
            this.renderConfigTable(table);
        } catch (error) {
